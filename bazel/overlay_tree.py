...
def main(args):
  for root, dirs, files in os.walk(args.overlay):
    ...
    # Create links in the target tree to overlay files.
    for file in files:
      relpath = os.path.join(rel_root, file)
      _symlink_abs(os.path.join(args.overlay, relpath),
                   os.path.join(args.target, relpath))

    # Create links in the target tree to source files.
    for src_entry in os.listdir(os.path.join(args.src, rel_root)):
      # Avoid files that were linked to overlay files and any directories.
      if src_entry not in files and src_entry not in dirs:
